...
def knight_tester(board, y, x) -> bool:
    ...
    return loc(board, y + 1, x + 2) == 'N' or \
        loc(board, y + 2, x + 1) == 'N' or \
        loc(board, y + 2, x - 1) == 'N' or \
        loc(board, y + 1, x - 2) == 'N' or \
        loc(board, y - 1, x - 2) == 'N' or \
        loc(board, y - 2, x - 1) == 'N' or \
        loc(board, y - 2, x + 1) == 'N' or \
        loc(board, y - 1, x + 2) == 'N'
...
def king_tester(board, y, x) -> bool:
    ...
    return loc(board, y - 1, x - 1) == 'K' or \
        loc(board, y - 1, x) == 'K' or \
        loc(board, y - 1, x + 1) == 'K' or \
        loc(board, y, x - 1) == 'K' or \
        loc(board, y, x + 1) == 'K' or \
        loc(board, y + 1, x - 1) == 'K' or \
        loc(board, y + 1, x) == 'K' or \
        loc(board, y + 1, x + 1) == 'K'
...
def projecting_piece_tester(board: list, y: int, x: int,
                            y_activator: int, x_activator: int,
                            piece: str) -> bool:
    '''Needs a lengthy doc string to define usage here.'''
    adder = 1
    while (loc(board, y + (y_activator * adder),
           x + (x_activator * adder)) == '.'):
        adder += 1
    if loc(board, y + (y_activator * adder),
       x + (x_activator * adder)) == piece:
        return True
    return False
...
def queen_tester(board, y, x) -> bool:
    ...
    return projecting_piece_tester(board, y, x, 0, 1, 'Q') or \
        projecting_piece_tester(board, y, x, 0, -1, 'Q') or \
        projecting_piece_tester(board, y, x, 1, 0, 'Q') or \
        projecting_piece_tester(board, y, x, -1, 0, 'Q') or \
        projecting_piece_tester(board, y, x, 1, -1, 'Q') or \
        projecting_piece_tester(board, y, x, -1, -1, 'Q') or \
        projecting_piece_tester(board, y, x, 1, 1, 'Q') or \
        projecting_piece_tester(board, y, x, -1, 1, 'Q')
...
def bk_in_check(board):
    ...
    return pawn_tester(board, king_y, king_x) or \
        knight_tester(board, king_y, king_x) or \
        king_tester(board, king_y, king_x) or \
        rook_tester(board, king_y, king_x) or \
        bishop_tester(board, king_y, king_x) or \
        queen_tester(board, king_y, king_x)
