...
if 'debugpy' in sys.modules:
    ...
    print('If you are using VSCode or another IDE, ensure you are not enabling debugging.')
...
class GraderTestResult:
    ...
    def attach_output(self, label, data=None, prefix='  '):
        self.output.append("{htag:s}<{tag:s}>".format(htag=GraderTestResult.HEADER_TAG, tag=label))
    ...
    def add_diff(self, label, actual, expect):
        ...
        self.output += list(difflib.Differ(charjunk=lambda _: False)
            .compare(expect.splitlines(), actual.splitlines()))
        self.output.append('  ')
...
def load_pycodestyle(pycodestyle_src):
    try:
        spec = importlib.util.spec_from_loader('pycodestyle', loader=None, origin="pycodestyle.py")
        ...
        class StoredReport(pycodestyle.BaseReport):
            ...
            def error(self, line_number, offset, text, check):
                ...
                if code is not None:
                    self._message_log.append((line_number, offset, code, text[5:], check.__doc__))
                return code
            ...
            def get_messages(self):
                ...
                for line_number, offset, code, text, doc in self._message_log:
                    messages.append(self._fmt.format(
        ...
        return pycodestyle.StyleGuide(max_line_length=120, reporter=StoredReport,
            ignore=IGNORED_STYLE_ERRORS)
    except:
        traceback.print_exc()
        pass
    return None
...
def run_interactive(cmd, stdin, source=None, tlimit=1_000_000):
    ...
    async def get_line(output, pipe):
        ...
        output.append(line.decode(encoding=sys.stdout.encoding, errors='backslashreplace'))
    ...
    async def __run():
        ...
        if runtime >= tlimit:
            err = "[{source:s}] maximum runtime allowance ({limit:.2f} seconds) exceeded".format(source=source, limit=tlimit)
            exitcode = None
...
def error_cleanup(msg):
    def repfn(m):
        return 'File "{base:s}", line {line:s}'.format(base=os.path.basename(m.group(1)), line=m.group(2))
...
def print_progress(label, x, n, failed=0, SIZE=30):
    if n == 0: return #raise GraderException("No tests found for grader.")
    fill = SIZE*x//n
    bar = chr(9608)*fill + chr(9617)*(SIZE-fill)
    desc = "(Test {num:03d}/{cnt:03d})".format(num=x, cnt=n)
    if failed > 0: desc += " ({:d} tests failed)".format(failed)
    print("\r{bar:s} [{perc:03d}%] {desc:s} - {label:s}".format(
        bar=bar, perc=100*x//n, desc=desc, label=label), end='')
    if x == n: print("")
    sys.stdout.flush()
...
def test_one(folder, srcpath=None, config=None, style=None, quiet=False):
    ...
    if not os.path.exists(srcpath):
        ...
        if len(m) > 0:
            message.append("This name looks close, perhaps a typo? {matches[0]:s}".format(matches=m))
        console_error('\n'.join(message))
        yield GraderReport(source, error=''.join(x+'\n' for x in message)).make_error()
        return
    ...
    def make_text_stream(x, config):
        ...
        return x if isinstance(x, io.TextIOWrapper) else io.TextIOWrapper(x, **encoding_flags)
    ...
    if custom_grader.exists():
        try:
            ...
            raise GraderException("Could not open custom grader for {src:s}".format(src=source))
    ...
    allowed_modules = [x.strip() for x in config.get('grader-config', 'allowed-modules', fallback='').split(',') if x]
    ...
    if illegal_found:
        ...
        yield GraderReport(source, error="Illegal imports: {:s}\n".format(','.join(illegal_found))).make_error()
        return
    ...
    report.soft_timeout = config.getint('grader-config', 'soft-timeout', fallback=report.timeout)
    ...
    if gmodule and hasattr(gmodule, 'grade'):
        ...
        for testnum, stdin_file in enumerate(stdin_files):
            ...
            if test_config_file.exists():
                test_config.read_string(test_config_file.read_text(), source=str(test_config_file))

            test_name = test_config.get('test-config', 'test-name', fallback='#'+test_id)
            ...
            with tempfile.TemporaryDirectory() as workspace:
                ...
                try:
                    ...
                    raise GraderException('\n'.join([
                        "Failed to setup input data for testing ({src:s}, test {test:s})".format(src=source, test=test_name),
                ...
                for file in folder.iterdir():
                    if file.name.startswith(input_file_tag):
                        ...
                        try:
                            with open(os.path.join(workspace, input_file_name), 'w') as inpc:
                                with file.open('r') as inpfd:
                                    shutil.copyfileobj(make_text_stream(inpfd, test_config), inpc)
                        except:
                            raise GraderException('\n'.join([
                                "Failed to setup input data for testing ({src:s}, test {test:s})".format(src=source, test=test_name),
                                "Issue with getting input file \"{fn:s}\" for test case:".format(fn=input_file_name),
                                traceback.format_exc(),
                                ]))
                ...
                try:
                    ...
                    with cwd(workspace):
                        exit_code, output_text, stderr, test_result.runtime = run_interactive(
                            [sys.executable, source], source=source, stdin=stdin_text, tlimit=report.timeout)
                        ...
                        if stdout_file.exists():
                            ...
                            with stdout_file.open('r') as expected_stream:
                                expect_output = make_text_stream(expected_stream, test_config)
                                result, feedback = check_output(actual_output, expect_output, type='stdout')
                                if not result:
                                    test_result.set_score(score=0.0, info="console output mismatch")
                                ...
                                if feedback is not None:
                                    ...
                                    test_result.add_diff(label, output_text, expect_output.read())
                        ...
                        for in_filename in input_raw_set:
                            ...
                            if os.path.getsize(input_file_name) < 10 * 1024:
                                with open(input_file_name, 'r') as inp_file:
                                    test_result.attach_output("input file: {file:s}".format(file=input_file_name),
                                        inp_file.readlines())

                        for out_filename in output_file_set:
                            if os.path.exists(out_filename):
                                with open(out_filename, 'r') as actual_output:
                                    zip_file = folder / '{id:s}-out-{base:s}'.format(id=test_id, base=out_filename)
                                    with zip_file.open('r') as expected_stream:
                                            expect_output = make_text_stream(expected_stream, test_config)
                                            result, feedback = check_output(actual_output, expect_output, type='file:'+out_filename)
                                            if not result:
                                                test_result.set_score(score=0.0,
                                                    info="output file contents incorrect: {file:s}".format(file=out_filename))

                                            label = "output file: {file:s}".format(file=out_filename)
                                            if feedback is not None:
                                                test_result.attach_output(label, feedback)
                                            else:
                                                for x in (actual_output, expect_output): x.seek(0)
                                                test_result.add_diff(label, actual_output.read(), expect_output.read())
                            else:
                                test_result.set_score(score=0.0, info="file expected, but missing: {file:s}".format(file=out_filename))
                                test_result.attach_output("output file missing: {file:s}".format(file=out_filename), "")

                        if exit_code != 0 or len(stderr) > 0:
                            clean_stderr = error_cleanup(stderr)
                            test_result.attach_output("runtime errors (exit code: {x:s})".format(x=str(exit_code)), clean_stderr)
                            test_result.set_score(score=0.0, info="runtime errors or nonstandard exit code")
                            report.errors.add(clean_stderr)
                except GraderException as e:
                    raise e
                except Exception as e:
                    traceback.print_exc()
                    raise GraderException("An error occurred when running the program ({src:s}) against test data".format(src=source))
...
def _get_report(results):
    ...
    if len(reports) != 1: raise GraderException('Cannot get singular report from mixed results.')
    for report in reports: return report
...
def _get_tests_from_folder(tests_folder):
    ...
    for folder in tests_folder.iterdir():
        if folder.is_dir() and folder.name.startswith('test-'):
            ...
            package_version = config.getint('grader-config', 'grader-version', fallback=0)
            if package_version > GRADER_VERSION:
                write_error('This test zip was compiled for a newer version of the grader.')
                write_error('Please download the newest version of {script:s} from AsULearn before continuing.'
...
def test_all(zippath):
    ...
    if this_version.major < PYTHON_VERSION_MAJOR:
        write_error(("Grading is done in Python {maj:d}.{min:d}. You are running Python " +
        ...
        write_error(("Grading is done in Python {maj:d}.{min:d}. You are running Python " +
            "{v.major:d}.{v.minor:d}.").format(maj=PYTHON_VERSION_MAJOR, min=PYTHON_VERSION_MINOR, v=this_version))
        write_error("You may encounter minor inconsistencies if you aren't running the correct version.")
    ...
    if not os.path.exists(zippath):
        ...
        write_error("Redownload the test zip and try again. If problems persist, notify your instructor.")
        return False
    ...
    with zipfile.ZipFile(zippath, 'r') as testzip:
        ...
        for folder,config in test_set:
            ...
            try:
                ...
                write_error(("If you believe you're seeing this message in error, " +
...
def make_submission_zip(testzip, sources):
    ...
    with zipfile.ZipFile(output, mode='w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as submitzip:
        for source in sources:
            submitzip.write(source)
    print("({output:s} generated in this folder for submission to AsULearn)".format(output=output))
    return output
...
def _run_tests(zippath, q):
    ...
    last = 'Submission zip created: "{output:s}"'.format(output=os.path.abspath(output)) if output else ''
    q.put((None, (last,100)))
...
def test_via_gui(testzip=None, autorun=False):
    ...
    text_area.tag_configure('header', background='black', foreground='white', font='Courier 10 bold')
    tag_map = {
        '- ': ('goodline', 'goodchar'),
        '+ ': ('badline', 'badchar'),
        '? ': ('infoline', None),
    ...
    def _add_style_errors(report):
        if report.style_errors:
            label = '<style errors ({count:d})>\n'.format(count=len(report.style_errors))
    ...
    def work_loop():
        def _handle_result(result):
            if result.get_name() in results_dict: return
            status = PASS_ICON if result.is_passing() else FAIL_ICON
            task_list.insert('', tk.END, image=status,
    ...
    test_path_select_btn = tk.Button(setup_frame, text="Choose File",
        command=select_file(TYPE_ZIP, testzip_path))
    ...
    window.state('zoomed')
    if autorun: load_zip() # if there is a zipfile
    window.mainloop()
...
def main():
    if len(sys.argv) < 2:
        ...
        if len(zips) > 1:
            print("Found multiple ({:d}) test zips, ignoring all but the most recent one!".format(len(zips)))
